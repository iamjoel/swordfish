...
<template>
  <main>
    <a-card class="general-card" :title="props.title">
      <SearchPanel @search="fetchList(1)" @reset="handleReset">
      ...
      >
        <template #operation="{ rowData }">
          <Space>
            <Button @click="handleOnEdit(rowData)" type="text" size="xs"
              >Edit</Button
            >
            <PopupConfirm
              @ok="handleDelete(rowData.id)"
              content="Confirm to Delete?"
            >
              <Button size="xs" status="danger" type="text">Delete</Button>
            </PopupConfirm>
          </Space>
        </template>
      </Table>
